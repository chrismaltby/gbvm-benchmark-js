...
(function (GameboyJS) {
  ...
  CPU.interruptRoutines = {
    0: function (p) {
      GameboyJS.cpuOps.RSTn(p, 0x40);
    },
    1: function (p) {
      GameboyJS.cpuOps.RSTn(p, 0x48);
    },
    2: function (p) {
      GameboyJS.cpuOps.RSTn(p, 0x50);
    },
    3: function (p) {
      GameboyJS.cpuOps.RSTn(p, 0x58);
    },
    4: function (p) {
      GameboyJS.cpuOps.RSTn(p, 0x60);
    },
  };
  ...
  CPU.prototype.frame = function () {
    if (!this.isPaused) {
      this.nextFrameTimer = setTimeout(
        ...
        1000 / GameboyJS.Screen.physics.FREQUENCY
      );
    }
    ...
    try {
      ...
      while (!vblank) {
        ...
        if (!this.isHalted) {
          if (this.onBeforeInstruction) {
            this.onBeforeInstruction();
          }
  ...
  CPU.prototype.fetchOpcode = function () {
    if (this.onFetchOpcodeCallback) {
      this.onFetchOpcodeCallback();
    }
    var opcode = this.memory.rb(this.r.pc++);
    if (opcode === undefined) {
      console.log(opcode + " at " + (this.r.pc - 1).toString(16));
      this.stop();
      return;
    }
...
(function (GameboyJS) {
  ...
  GPU.prototype.drawBackground = function (LCDC, line, lineBuffer) {
    ...
    var mapStart = GameboyJS.Util.readBit(LCDC, 3)
      ? GPU.tilemap.START_1
      : GPU.tilemap.START_0;
    ...
    for (
      ...
      var tileData =
        ...
        (this.bgTileCache[tileIndex] = this.readTileData(tileIndex, dataStart));
  ...
  // Copy a tile line from a tileBuffer to a line buffer, at a given x position
  GPU.prototype.copySpriteTileLine = function (
    lineBuffer,
    tileBuffer,
    x,
    palette
  ...
  GPU.prototype.drawTile = function (
    tileData,
    x,
    y,
    buffer,
  ...
  GPU.prototype.drawWindow = function (LCDC) {
    ...
    var mapStart = GameboyJS.Util.readBit(LCDC, 6)
      ? GPU.tilemap.START_1
      : GPU.tilemap.START_0;
...
(function (GameboyJS) {
  ...
  Input.prototype.update = function () {
    ...
    if (value & 0x10) {
      // direction keys listened
      value |= this.state & 0x0f;
    } else if (value & 0x20) {
      // action keys listened
...
(function (GameboyJS) {
  ...
  var ops = {
    LDrrnn: function (p, r1, r2) {
      p.wr(r2, p.memory.rb(p.r.pc));
      p.wr(r1, p.memory.rb(p.r.pc + 1));
      p.r.pc += 2;
      p.clock.c += 12;
    },
    LDrrar: function (p, r1, r2, r3) {
      ops._LDav(p, GameboyJS.Util.getRegAddr(p, r1, r2), p.r[r3]);
      p.clock.c += 8;
    },
    LDrrra: function (p, r1, r2, r3) {
      p.wr(r1, p.memory.rb(GameboyJS.Util.getRegAddr(p, r2, r3)));
      p.clock.c += 8;
    },
    LDrn: function (p, r1) {
    ...
    LDrr: function (p, r1, r2) {
      p.wr(r1, p.r[r2]);
      p.clock.c += 4;
    },
    LDrar: function (p, r1, r2) {
      p.memory.wb(p.r[r1] + 0xff00, p.r[r2]);
      p.clock.c += 8;
    },
    LDrra: function (p, r1, r2) {
    ...
    LDsprr: function (p, r1, r2) {
      p.wr("sp", GameboyJS.Util.getRegAddr(p, r1, r2));
      p.clock.c += 8;
    },
    LDnnar: function (p, r1) {
      var addr = (p.memory.rb(p.r.pc + 1) << 8) + p.memory.rb(p.r.pc);
      p.memory.wb(addr, p.r[r1]);
      p.r.pc += 2;
      p.clock.c += 16;
    },
    LDrnna: function (p, r1) {
      var addr = (p.memory.rb(p.r.pc + 1) << 8) + p.memory.rb(p.r.pc);
    ...
    ADDrr: function (p, r1, r2) {
      var n = p.r[r2];
      ops._ADDrn(p, r1, n);
      p.clock.c += 4;
    },
    ADDrn: function (p, r1) {
      var n = p.memory.rb(p.r.pc++);
      ops._ADDrn(p, r1, n);
      p.clock.c += 8;
    },
    ...
    ADDrrrr: function (p, r1, r2, r3, r4) {
      ops._ADDrrn(p, r1, r2, (p.r[r3] << 8) + p.r[r4]);
      p.clock.c += 8;
    },
    ADDrrsp: function (p, r1, r2) {
    ...
    ADCrrra: function (p, r1, r2, r3) {
      var n = p.memory.rb(GameboyJS.Util.getRegAddr(p, r2, r3));
      ops._ADCrn(p, r1, n);
      p.clock.c += 8;
    },
    ...
    SUBrra: function (p, r1, r2) {
      var n = p.memory.rb(GameboyJS.Util.getRegAddr(p, r1, r2));
      ops._SUBn(p, n);
      p.clock.c += 8;
    },
    _SUBn: function (p, n) {
    ...
    SBCrra: function (p, r1, r2) {
      var v = p.memory.rb((p.r[r1] << 8) + p.r[r2]);
      ops._SBCn(p, v);
      p.clock.c += 8;
    },
    _SBCn: function (p, n) {
    ...
    ANDrra: function (p, r1, r2) {
      p.r.A &= p.memory.rb(GameboyJS.Util.getRegAddr(p, r1, r2));
      p.r.F = p.r.A == 0 ? 0xa0 : 0x20;
      p.clock.c += 8;
    },
    XORr: function (p, r1) {
    ...
    RRCrra: function (p, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      p.r.F = 0;
      var out = p.memory.rb(addr) & 0x01;
      if (out) p.r.F |= 0x10;
    ...
    RLCr: function (p, r1) {
      p.r.F = 0;
      var out = p.r[r1] & 0x80 ? 1 : 0;
      if (out) p.r.F |= 0x10;
      p.r[r1] = ((p.r[r1] << 1) + out) & 0xff;
      if (p.r[r1] == 0) p.r.F |= 0x80;
      p.clock.c += 4;
    },
    RLCrra: function (p, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      p.r.F = 0;
      var out = p.memory.rb(addr) & 0x80 ? 1 : 0;
      if (out) p.r.F |= 0x10;
    ...
    RLrra: function (p, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      var c = p.r.F & 0x10 ? 1 : 0;
      p.r.F = 0;
      var out = p.memory.rb(addr) & 0x80;
      out ? (p.r.F |= 0x10) : (p.r.F &= 0xef);
    ...
    SRArra: function (p, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      p.r.F = 0;
      if (p.memory.rb(addr) & 0x01) p.r.F |= 0x10;
      var msb = p.memory.rb(addr) & 0x80;
    ...
    BITir: function (p, i, r1) {
      var mask = 1 << i;
      var z = p.r[r1] & mask ? 0 : 1;
      var f = p.r.F & 0x10;
      f |= 0x20;
      if (z) f |= 0x80;
      p.r.F = f;
    ...
    BITirra: function (p, i, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      var mask = 1 << i;
      var z = p.memory.rb(addr) & mask ? 0 : 1;
      var f = p.r.F & 0x10;
      f |= 0x20;
      if (z) f |= 0x80;
      p.r.F = f;
    ...
    SETir: function (p, i, r1) {
      var mask = 1 << i;
      p.r[r1] |= mask;
      p.clock.c += 4;
    },
    SETirra: function (p, i, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      var mask = 1 << i;
      p.memory.wb(addr, p.memory.rb(addr) | mask);
      p.clock.c += 12;
    },
    RESir: function (p, i, r1) {
      var mask = 0xff - (1 << i);
      p.r[r1] &= mask;
      p.clock.c += 4;
    },
    RESirra: function (p, i, r1, r2) {
      var addr = GameboyJS.Util.getRegAddr(p, r1, r2);
      var mask = 0xff - (1 << i);
      p.memory.wb(addr, p.memory.rb(addr) & mask);
      p.clock.c += 12;
    ...
    SWAPrra: function (p, r1, r2) {
      var addr = (p.r[r1] << 8) + p.r[r2];
    ...
    JPrr: function (p, r1, r2) {
      p.r.pc = (p.r[r1] << 8) + p.r[r2];
      p.clock.c += 4;
      if (p.gameboy.cpu.onJumpCallback) {
        p.gameboy.cpu.onJumpCallback();
      }
    },
    JRn: function (p) {
      var v = p.memory.rb(p.r.pc++);
    ...
    RET: function (p) {
      ...
      p.wr("sp", p.r.sp + 1);
      p.clock.c += 16;
      if (p.gameboy.cpu.onReturnCallback) {
        p.gameboy.cpu.onReturnCallback();
      }
    },
    RETcc: function (p, cc) {
      if (GameboyJS.Util.testFlag(p, cc)) {
    ...
    _CALLnn: function (p) {
      ...
      p.r.pc = j;
      if (p.gameboy.cpu.onCallCallback) {
        p.gameboy.cpu.onCallCallback();
      }
    },
    CPL: function (p) {
      p.wr("A", ~p.r.A & 0xff);
...
(function (GameboyJS) {
  ...
  var Gameboy = function (canvas, options) {
    ...
    this.statusContainer =
      document.getElementById(this.options.statusContainerId) ||
      document.createElement("div");
    this.gameNameContainer =
      document.getElementById(this.options.gameNameContainerId) ||
      document.createElement("div");
    this.errorContainer =
      document.getElementById(this.options.errorContainerId) ||
      document.createElement("div");
      */
  };
  ...
  MBC3.prototype.manageWrite = function (addr, value) {
    switch (addr & 0xf000) {
      ...
      case 0x7000: // Latch clock data
        throw new GameboyJS.UnimplementedException(
...
(function (GameboyJS) {
  ...
  Memory.prototype.reset = function () {
    ...
    for (
      var i = Memory.addresses.VRAM_START;
    ...
    for (
      var i = Memory.addresses.DEVICE_START;
  ...
  Memory.prototype.deviceram = function (address, value) {
    if (
      address < Memory.addresses.DEVICERAM_START ||
      address > Memory.addresses.DEVICERAM_END
    ) {
  ...
  APU.prototype.manageWrite = function (addr, value) {
    ...
    switch (addr) {
      ...
      case 0xff26:
        ...
        if (!this.enabled) {
          for (var i = 0xff10; i < 0xff27; i++) this.memory[i] = 0;
          // todo stop sound
        }
        break;
    }
...
(function (GameboyJS) {
  ...
  Channel3.prototype.getFrequency = function () {
    var freq =
      2048 - 65536 / (this.bufferSource.playbackRate.value * this.baseSpeed);
    return freq | 1;
  };
